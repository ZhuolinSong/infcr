...
pspline = function (data, argvals.new = NULL, weight=c("SUBJ","SOB","aSUBJ"), knots = 15, p = 3, m = 2, 
                    lambda = NULL, search.length = 100, lower = -20, upper = -3, knots.option = "equally-spaced") {
  Y <- data$y
  t <- data$argvals
  subj <- data$subj
  tnew <- argvals.new
  ...
  if(length(weight) != J){
    print("Inconsistent weight length with data y!")
    weight = rep(1/J, J)
  }
...
pspline_setting = function(x,knots=select_knots(x,15),p=3,m=2,periodicity=FALSE,weight=NULL){
